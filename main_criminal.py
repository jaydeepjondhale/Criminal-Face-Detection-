...
def TakeImages():
    ...
    assure_path_exists("CriminalDetails/")
    assure_path_exists("TrainingImage/")
    serial = 0
    exists = os.path.isfile("CriminalDetails\CriminalDetails.csv")
    if exists:
        with open("CriminalDetails\CriminalDetails.csv", 'r') as csvFile1:
        ...
        with open("CriminalDetails\CriminalDetails.csv", 'a+') as csvFile1:
    ...
    if ((name.isalpha()) or (' ' in name)):
        ...
        cam = cv2.VideoCapture('http://192.168.2.2:8080/video')# For IP Camera
        ...
        while (True):
            ...
            for (x, y, w, h) in faces:
                ...
                cv2.imwrite("db\ " + name + "." + str(serial) + "." + Id + '.' + str(sampleNum) + ".jpg",
                            gray[y:y + h, x:x + w])
                # display the frame
        ...
        row = [serial, '', Id, '', name]
        with open('CriminalDetails\CriminalDetails.csv', 'a+') as csvFile:
...
def TrackImages():
    ...
    exists1 = os.path.isfile("CriminalDetails\CriminalDetails.csv")
    if exists1:
        df = pd.read_csv("CriminalDetails\CriminalDetails.csv")
    else:
        mess._show(title='Details Missing', message='Criminal details are missing, please check!')
        cam.release()
        cv2.destroyAllWindows()
        window.destroy()
    while True:
...
window.title("Criminal Verification")
...
message3 = tk.Label(window, text="Criminal Verification" ,fg="white",bg="#262523" ,width=55 ,height=1,font=('times', 29, ' bold '))
...
trackImg = tk.Button(frame1, text="Start Verification", command=TrackImages  ,fg="black"  ,bg="yellow"  ,width=35  ,height=1, activebackground = "white" ,font=('times', 15, ' bold '))
